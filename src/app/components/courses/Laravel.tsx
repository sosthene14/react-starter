...
export default function Laravel() {
  ...
  const getGroups = useCallback(async () => {
    ...
    const response = await fetch(
      `https://courses-list-backend.vercel.app/api/v1/get-groups/laravel`,
      {
        method: "GET",
        headers: {
  ...
  const exportPDF = () => {
    ...
    doc.text("Liste des groupes des étudiants Laravel", 20, yPosition);
    yPosition += 10;
    ...
    doc.save("laravel_students_list.pdf");
  };
  ...
  const handleAddGroup = async () => {
    if (students.length > 3) {
      message.warning(3 + "étudiants maximum");
      return;
    }
    setIsLoading(true);
    const dataToUpload = {
